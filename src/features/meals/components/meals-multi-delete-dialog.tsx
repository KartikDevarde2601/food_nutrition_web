...
type MealsMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function MealsMultiDeleteDialog<TData extends Meal>({
  open,
  onOpenChange,
  table,
}: MealsMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = async () => {
    ...
    const deletePromises = selectedRows.map((row) =>
      deleteMeal(row.original.mealId)
    )
    ...
    <ConfirmDialog
      ...
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            Are you sure you want to delete the selected meals? <br />
